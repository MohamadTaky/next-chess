...
export default function usePusherCallbacks() {
  ...
  useEffect(() => {
    ...
    return () => {
      channel.unbind("move", moveHandler);
      channel.unbind("promote", promotionHandler);
      channel.unbind("incoming-message", incomingMessageHandler);
      channel.unbind("pusher:member_added", opponentInfoHandler);
      channel.unsubscribe();
      initGameSlice();
      initRoomSlice();
    };
